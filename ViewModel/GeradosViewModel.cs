...
{
    public class GeradosViewModel : ViewModelBase
    {
        private List<Gerados> GetGerados = new List<Gerados>();
            ...
            try
            {
                if (selectedIndex == 1)
                {
                    ...
                    for (int i = 0; i < strings.Length; i++)
                    {
                        ...
                        box.Tag = i;
                        box.IsChecked = Data.Data.SortingGerados == i;
                        box.Click += (s, a) =>
                        {

                            dialog.Hide();
                            RadioButton btn = (RadioButton)s as RadioButton; 
                            Data.Data.SortingGerados = (int)btn.Tag;
                            loadLista(); 
        ...
        internal async void DeleteAll(int selectedIndex)
        {
            try
            {
                if (selectedIndex == 1)
                {
                    ...
                    dialog.Content = loader.GetString("deleteAllSubtitleGerados");
                    ...
                    dialog.SecondaryButtonClick += async (s, a) =>
                    {
                        ...
                        {
                            dialog.Hide();
                            await App.GetInstance.GetDatabase.Child($"{Data.Conta.UUID}/Gerados").DeleteAsync();
                            loadLista();
                            CallingInfoInstance_CellInfoUpdateCompletedAsync();
        ...
        private void OnEditarSalvar()
        {
            ...
            {
                gerado.DisplayName = remonear;
                gerados.Atualizar(gerado);
                OnNuvemEditor(gerado);
                dialog.Hide();
            }

        }

        private async void OnNuvemEditor(Gerados gerados)
        {
            try
            {
                if (gerado.IsNuvem)
        ...
        public async void OnDelete(Gerados gerado)
        {
            try
            {
                ContentDialog dialog = new ContentDialog();
                dialog.Title = gerado.DisplayName;
        ...
        private async void OnDeleteNuvem(Gerados gerado)
        {
            try
            {
                if (gerado.IsNuvem)
                {
                    ...
                    { 
                        var db = App.GetInstance.GetDatabase;
                        await db.Child($"{Data.Conta.UUID}/Gerados/{gerado.UUID}").DeleteAsync();
                    }
                }
            }
            catch { }
        ...
        public async void OnEditar(Gerados gerado)
        {
            try
            {
                this.gerado = gerado;
        ...
        public List<Gerados> Lista
        {
            get { return GetGerados; }
            private set
            {
                if (GetGerados != value)
                {
